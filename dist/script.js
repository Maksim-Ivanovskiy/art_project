...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_drop__WEBPACK_IMPORTED_MODULE_12__["drop"])();
});
...
var forms = function forms() {
  ...
  var message = {
    ...
    //создаем объект с сообщениями, которые будут показываться пользователю
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      //перебираем все формы, навешиваем обработчик события, создаем блок, в котором будем показывать пользователю, что что-то пошло не так, либо запрос отправился.
      e.preventDefault();
      ...
      var formData = new FormData(item); //собираем все введенные данные из этой формы   
      ...
      Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData) //отправляем запрос на сервер по адресу в первом документе с даннымы, который получил formData
      .then(function (res) {
        console.log(res);
  ...
  /*     const element = document.documentElement,
      ...
      const calcScroll = () => {
          upElem.addEventListener('click', function(event) {
              ...
              if(this.hash !== '') {
                  ...
                  while (hashElement.offsetParent) { //цикл позволит перебрать всех родителей элемента и узнать сколько пикселей нужно отлистать
                      hashElementTop += hashElement.offsetTop; //позволяет определить сколько пикселей осталось до верхней границы род элемента от хэш
      ...
      const smoothScroll= (from, to, hash) => {
          ...
          let move = setInterval(function () {
              ...
              if (
                  previousScrollTop === scrollTop || (to > from && scrollTop >= to) || (to < from && scrollTop <= to)
              ) {
                  clearInterval(move);
                  history.replaceState(history.state, document.title, location.href.replace(/#.*$/g, '') + hash);
                  //с помощью регулярного выражения мы найдем все шарпы в конце строки href
              } else {
